...
2. **Creates `.claude/settings.json`** -- registers specflow as an MCP server so Claude Code can use `sf_*` tools automatically.
...
1. Create `.claude/settings.json` in your project root (or edit the existing one):
...
| Entity | Use case | Example |
|--------|----------|---------|
| **Initiative** | Strategic goal grouping multiple epics | "Launch v2.0" |
| **Epic** | A shippable feature or capability | "Auth system", "Payment integration" |
| **Story** | Atomic work unit with acceptance criteria | "Create JWT middleware", "Add rate limiting" |
| **PRD** | Problem, users, goals, scope, requirements | Product spec for the auth system |
| **Tech Spec** | Architecture, data model, API design | How JWT tokens flow through the system |
| **ADR** | Architecture Decision Record | "Why JWT over session cookies" |
| **Decision** | Lightweight inline decision | Quick choices recorded during planning |
